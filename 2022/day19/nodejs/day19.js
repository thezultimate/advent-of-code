...
const day19Part2 = (inputArr) => {
  ...
  let threeMultiplied = 1;
  for (const x of maxGeodeList) {
    threeMultiplied *= x;
  }

  return threeMultiplied;
};

const processBlueprint2 = (
  blueprint,
  minute,
  ore,
  clay,
  ...
  const remainingMinutes = 33 - minute;
  let buildMinutes;

  // Check max possible geodes for the current situation. If less than maxGeode, stop.
  let maxPossibleGeodes =
    geode +
    geodeRobotOwned * remainingMinutes +
    ((1 + remainingMinutes - 1) * (remainingMinutes - 1)) / 2; // Managed to remember arithmetic sequence from high school
  if (maxPossibleGeodes < maxGeode) return;
