...
const day24Part1 = (inputArr) => {
  ...
  let rootNode = {
    startPositionMap: startPositionMap,
    xCurrent: xCurrent,
    yCurrent: yCurrent,
    step: step,
    xEnd: xEnd,
...
const move = (
  ...
  if (xCurrent === xEnd && yCurrent === yEnd) {
    if (step - 1 < minSteps) minSteps = step - 1;
    minStepsFound = true;
    return;
  }

  // This is very slow. There must be cycles.
  // Precomputed nextPositionMap would be much more efficient than computing one for every movement!
  // The program terminates with correct result anyway.
  let nextPositionMap = {};
